...
app.post('/users', (req, res) => {
  ...
  if (!userName || !email || !avatar) {
    return res.status(404).json({
      message: 'Missing require fields: username, email, avatar',
    });
  }
  ...
  if (existingUser) {
    return res.json({
      message: 'Email is already existed, please try with another email ',
    });
  }
...
// Create new post for a specific user
...
app.post('/posts/:userId', (req, res) => {
  ...
  if (!content || !isPublic) {
    return res.status(404).json({
      message: 'Missing required fields: content, isPublic',
    });
  }
